    ...
    if filter_type is None:
        ...
        if filter_type == 'low':
            if filt_freq is None:
                logging.warning('Upper pass-band freq is not given but is required. Default parameters' 
                                'for low pass filtering will be used')
                filt_freq = 30    # Default
            EEG_filt = mne.filter.filter_data(EEG, sfreq, None, filt_freq, picks=None,
                                              filter_length=filter_length, l_trans_bandwidth='auto',
        ...
        if filter_type == 'high':
            if filt_freq is None:
                logging.warning('Lower pass-band freq is not given but is required. Default parameters'
                                'for high pass filtering will be used')
                filt_freq = 0.5    # Default
            EEG_filt = mne.filter.filter_data(EEG, sfreq, filt_freq, None,
        ...
        if filter_type == 'notch':
            if filt_freq is None:
                logging.warning('Frequency for notch filter is not complete.'
                                'The default will be used.')
                filt_freq = 60    # Default
            EEG_filt = mne.filter.notch_filter(EEG, sfreq, filt_freq,
