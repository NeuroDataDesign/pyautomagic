...
def calcQuality(data: np.ndarray, bad_chans: List, overallThresh: float = 50, timeThresh: float = 25,
    ...
    if not isinstance(overallThresh, int) and not isinstance(overallThresh, float) or isinstance(overallThresh, bool) or (overallThresh <= 0):
        overallThresh = 50
        logger.log(30,'Invalid overallThresh value. Default of 50 used.')
    if not isinstance(timeThresh, int) and not isinstance(timeThresh, float) or isinstance(timeThresh, bool) or (timeThresh <= 0):
        timeThresh = 25
        logger.log(30,'Invalid timeThresh value. Default of 25 used.')
    if not isinstance(chanThresh, int) and not isinstance(chanThresh, float) or isinstance(chanThresh,bool) or (chanThresh <= 0):
        chanThresh = 25
